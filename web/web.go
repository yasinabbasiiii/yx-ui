...
func (s *Server) startTask() {
	...
	s.cron.AddJob("@every 60s", job.NewCheckXrayRunningJob()) //samyar

	// Check if xray needs to be restarted every 30 seconds
	s.cron.AddFunc("@every 60s", func() { //samyar
	...
	go func() {
		...
		r := rand.New(source)
		randomNum := r.Intn(101) + 500 // تولید عدد تصادفی بین 800 تا 1000
	...
	//s.cron.AddJob("@every 3600s", job.NewCheckClientIpJob()) //Samyar
	...
	if (err == nil) && (isTgbotenabled) {
		...
		s.cron.AddJob("@every 120m", job.NewCheckHashStorageJob())

		// Check CPU load and alarm to TgBot if threshold passes
		cpuThreshold, err := s.settingService.GetTgCpu()
		if (err == nil) && (cpuThreshold > 0) {
			s.cron.AddJob("@every 60m", job.NewCheckCpuJob())
		}
	} else {
		s.cron.Remove(entry)
	}
